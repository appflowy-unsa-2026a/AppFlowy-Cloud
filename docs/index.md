# Proyecto IPS 2026-A — AppFlowy Cloud

**Curso:** Ingeniería y Procesos de Software  
**Semestre:** 2026-A  
**Universidad:** Universidad Nacional de San Agustín de Arequipa  
**Facultad:** Ingeniería de Producción y Servicios  

---

## Producto de software seleccionado

**AppFlowy Cloud**  
Backend self-hosted del workspace colaborativo open source AppFlowy (alternativa a Notion).

### Detalles principales

- **Repositorio original:** https://github.com/AppFlowy-IO/AppFlowy-Cloud  
- **Fork del equipo:** https://github.com/appflowy-unsa-2026a/AppFlowy-Cloud  
- **Licencia:** AGPL-3.0 (aprobada por el docente)  
- **Stack tecnológico:**  
  - Rust (86.7%)  
  - TypeScript  
  - Docker Compose  
- **Último release:** v0.11.9 — 12.MAY.2026  

---

## Documentación del proyecto

- [Plan de proyecto](plan-proyecto.md)  
- [Cronograma de sprints](cronograma.md)  
- [Equipo de trabajo](equipo.md)  

---

## Enlaces

- [GitHub Project (tablero Scrum)](https://github.com/orgs/appflowy-unsa-2026a/projects/1)  
- [Issues / Product Backlog](https://github.com/appflowy-unsa-2026a/AppFlowy-Cloud/issues)  
